...
const Modal = () => {
    ...
    const sendComment = async (e) => {
        ...
        const cleanData = {
            comment: input || '', // Ensure `input` is defined
            username: session.user?.name || 'Unknown', // Ensure `username` is defined
            tag: session.user?.tag || '', // Ensure `tag` is defined
            userImg: session.user?.image || '', // Ensure `userImg` is defined
            timestamp: serverTimestamp(),
        };

        try {
            await addDoc(collection(db, "posts", appContext.postId, "comments"), cleanData);
